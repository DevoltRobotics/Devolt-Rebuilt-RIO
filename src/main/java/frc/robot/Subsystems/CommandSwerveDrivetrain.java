...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
    ...
    public void allignToPose(Pose2d pose, CommandSwerveDrivetrain drivetrain){
        PIDController xpid = new PIDController(1, .01, 0);
        PIDController ypid = new PIDController(1, .01, 0);
        ...
        rotpid.setSetpoint(pose.getRotation().getDegrees());

        drivetrain.setControl(TunerConstants.cdrive.withForwardPerspective(ForwardPerspectiveValue.BlueAlliance)
            .withVelocityX(xpid.calculate(getState().Pose.getX())*TunerConstants.MaxSpeed)
            .withVelocityY(ypid.calculate(getState().Pose.getY())*TunerConstants.MaxSpeed)
            .withRotationalRate(rotpid.calculate(getState().Pose.getRotation().getDegrees())*TunerConstants.MaxAngularRate));
